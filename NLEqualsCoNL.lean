...
Lean version: leanprover/lean4:v4.24.0
Mathlib version: f897ebcf72cd16f89ab4577d0c826cd14afaafc7
This project request had uuid: 566b5c15-467d-4a4c-8b0d-0e5e4e88cbe9
...
/-
We have formalized the Immerman-Szelepcsényi theorem, proving that nondeterministic space complexity classes are closed under complementation for space bounds $S(n) \ge \log n$. Specifically, we proved `InNSPACE_O_eq_InCoNSPACE_O`, which states that $\text{NSPACE}(O(f)) = \text{co-NSPACE}(O(f))$ for $f(n) \ge \log n$. As a corollary, we derived `NL_eq_coNL`, confirming that $\text{NL} = \text{coNL}$. The proof relies on the construction of a "normalized" Turing machine and the inductive counting argument to verify non-reachability.
-/
...
Lean version: leanprover/lean4:v4.24.0
Mathlib version: f897ebcf72cd16f89ab4577d0c826cd14afaafc7
This project request had uuid: b6f15c1c-776a-4f1c-88a5-e5b9d15267c2
...
Lean version: leanprover/lean4:v4.24.0
Mathlib version: f897ebcf72cd16f89ab4577d0c826cd14afaafc7
This project request had uuid: a3527a59-ca9a-49ba-9dcb-73cb50e9b40e
...
/-
Definition of DualMachine that accepts when M gets stuck in a non-accepting state.
-/
def DualMachine {k : ℕ} [NeZero k] {σ : Type} [DecidableEq σ] [Inhabited σ] [Fintype σ]
  (M : TuringMachine k σ) : TuringMachine k σ :=
  have : Fintype M.V := M.fintypeV
  have : DecidableEq M.V := M.decV
  let V' := Option M.V
  let keep_w (r : Fin k → σ) : Fin (k-1) → σ := fun i => r ⟨i.val + 1, by omega⟩
  let is_stuck (u : M.V) (r : Fin k → σ) : Prop := ∀ v w m, (u, v, r, w, m) ∉ M.edges
  let extra_edges : Set (V' × V' × (Fin k → σ) × (Fin (k-1) → σ) × (Fin k → Move)) :=
    { e | ∃ (u : M.V) (r : Fin k → σ),
          u ≠ M.acceptState ∧ is_stuck u r ∧
          e = (some u, none, r, keep_w r, fun _ => Move.Stay) }
  let original_edges : Set (V' × V' × (Fin k → σ) × (Fin (k-1) → σ) × (Fin k → Move)) :=
    { e | ∃ (u v : M.V) (r : Fin k → σ) (w : Fin (k-1) → σ) (m : Fin k → Move),
          e = (some u, some v, r, w, m) ∧ (u, v, r, w, m) ∈ M.edges }
  { V := V',
    decV := inferInstance,
    fintypeV := inferInstance,
    edges := original_edges ∪ extra_edges,
    startState := some M.startState,
    acceptState := none,
    rejectState := some M.rejectState
  }
...
/-
Definition of DualMachine_v3 that accepts when M gets stuck in a non-accepting state.
-/
def DualMachine_v3 {k : ℕ} [NeZero k] {σ : Type} [DecidableEq σ] [Inhabited σ] [Fintype σ]
  (M : TuringMachine k σ) : TuringMachine k σ :=
  haveI : Fintype M.V := M.fintypeV
  haveI : DecidableEq M.V := M.decV
  let V' := Option M.V
  let edges' : Set (V' × V' × (Fin k → σ) × (Fin (k-1) → σ) × (Fin k → Move)) :=
    { e | ∃ (u v : M.V) (r : Fin k → σ) (w : Fin (k-1) → σ) (m : Fin k → Move),
          e = (some u, some v, r, w, m) ∧ (u, v, r, w, m) ∈ M.edges } ∪
    { e | ∃ (u : M.V) (r : Fin k → σ),
          u ≠ M.acceptState ∧ IsStuck M u r ∧
          e = (some u, none, r, fun i => r ⟨i.val+1, by omega⟩, fun _ => Move.Stay) }
  { V := V',
    decV := inferInstance,
    fintypeV := inferInstance,
    edges := edges',
...
/-
Definition of DualMachine_v4 using Sum type for states.
-/
def DualMachine_v4 {k : ℕ} [NeZero k] {σ : Type} [DecidableEq σ] [Inhabited σ] [Fintype σ]
  (M : TuringMachine k σ) : TuringMachine k σ :=
  haveI : Fintype M.V := M.fintypeV
  haveI : DecidableEq M.V := M.decV
  let V' := Sum M.V Unit
  let edges' : Set (V' × V' × (Fin k → σ) × (Fin (k-1) → σ) × (Fin k → Move)) :=
    ...
    { e | ∃ (u : M.V) (r : Fin k → σ),
          u ≠ M.acceptState ∧ IsStuck M u r ∧
          e = (Sum.inl u, Sum.inr (), r, fun i => r ⟨i.val+1, by omega⟩, fun _ => Move.Stay) }
  { V := V',
    decV := inferInstance,
    fintypeV := inferInstance,
    edges := edges',
    startState := Sum.inl M.startState,
    acceptState := Sum.inr (),
    rejectState := Sum.inl M.rejectState
  }
...
Definition of a normalized Turing machine that wraps an existing machine to ensure well-behaved accept/reject states.
-/
def NormalizedTM_v2 {k : ℕ} {σ : Type} [DecidableEq σ] [Inhabited σ] [NeZero k]
  (M : TuringMachine k σ) : TuringMachine k σ :=
  haveI : Fintype M.V := M.fintypeV
  haveI : DecidableEq M.V := M.decV
  let V' := Sum M.V (Fin 2)
...
-/
lemma NormalizedTM_v2_Reachable_StrongCorrespondence {k : ℕ} {σ : Type} [DecidableEq σ] [Inhabited σ] [NeZero k]
  (M : TuringMachine k σ) (input : List σ) (c' : Configuration k σ (NormalizedTM_v2 M).V) :
  Reachable (NormalizedTM_v2 M) (InitialConfig (NormalizedTM_v2 M) input) c' →
  ((∃ u, c'.state = Sum.inl u ∧ ∃ c, Reachable M (InitialConfig M input) c ∧ c.state = u ∧ c.tapes = c'.tapes ∧ c.positions = c'.positions) ∨
   (∃ x, c'.state = Sum.inr x ∧ ∃ c, Reachable M (InitialConfig M input) c ∧ c'.positions = c.positions)) := by
     ...
     · rename_i h₁ h₂ h;
       unfold step at h₂;
       rcases h with ( ⟨ u, hu, c, hc, hc', hc'', hc''' ⟩ | ⟨ x, hx, c, hc, hc' ⟩ ) <;> simp_all +decide [ NormalizedTM_v2 ];
...
-/
lemma NormalizedTM_v2_SpaceBounded {k : ℕ} {σ : Type} [DecidableEq σ] [Inhabited σ] [NeZero k]
  (M : TuringMachine k σ) (S : ℕ → ℕ) :
  SpaceBounded M S → SpaceBounded (NormalizedTM_v2 M) S := by
    intro hM c hc hc';
    -- By `NormalizedTM_v2_Reachable_StrongCorrespondence`, there exists a configuration `c'` reachable in `M` such that `hc.positions = c'.positions`.
    obtain ⟨c', hc'_reachable, hc'_positions⟩ : ∃ c' : Configuration k σ M.V, Reachable M (InitialConfig M c) c' ∧ hc.positions = c'.positions := by
      have := NormalizedTM_v2_Reachable_StrongCorrespondence M c hc hc';
      grind;
    aesop
...
-/
lemma NormalizedTM_v2_AcceptsN_iff {k : ℕ} {σ : Type} [DecidableEq σ] [Inhabited σ] [NeZero k]
  (M : TuringMachine k σ) (input : List σ) :
  AcceptsN (NormalizedTM_v2 M) input ↔ AcceptsN M input := by
    apply Iff.intro
    intro h_accepts_normalized
    obtain ⟨c', hc'_reachable, hc'_accept⟩ : ∃ c', Reachable (NormalizedTM_v2 M) (InitialConfig (NormalizedTM_v2 M) input) c' ∧ c'.state = Sum.inr 0 := by
      exact h_accepts_normalized
    generalize_proofs at *; (
    -- Since $c'$ is in state $Sum �.in�r 0$, it must have been reached from $Sum.inl M.acceptState$.
    obtain ⟨c', hc'_reachable, hc'_accept⟩ : ∃ c', Reachable (NormalizedTM_v2 M) (InitialConfig (NormalizedTM_v2 M) input) c' ∧ c'.state = Sum.inl M.acceptState := by
      obtain ⟨c'', hc''_reachable, hc''_accept⟩ : ∃ c'', Reachable (NormalizedTM_v2 M) (InitialConfig (NormalizedTM_v2 M) input) c'' ∧ c''.state = Sum.inr 0 ∧ c''.state = Sum.inr 0 := by
        aesop
        skip
      generalize_proofs at *; (
      ...
      · rename_i h₁ h₂ h
        generalize_proofs at *; (
        unfold step at h₂; simp_all +decide [ NormalizedTM_v2 ] ;
        grind))
    generalize_proofs at *; (
    have := NormalizedTM_v2_Reachable_StrongCorrespondence M input c' hc'_reachable; simp_all +decide [ AcceptsN ] ;
    grind +ring));
    intro h_accept
    obtain ⟨c, hc⟩ := h_accept;
    -- Since $c$ is reachable in $M$, we can construct a path in $NormalizedTM_v2 M$ that reaches $Sum.in �l� c$.
    have h_path : Reachable (NormalizedTM_v2 M) (InitialConfig (NormalizedTM_v2 M) input) (Configuration.mk (Sum.inl c.state) c.tapes c.positions) := by
      have h_path : ∀ c, Reachable M (InitialConfig M input) c → Reachable (NormalizedTM_v2 M) (InitialConfig (NormalizedTM_v2 M) input) (Configuration.mk (Sum.inl c.state) c.tapes c.positions) := by
        intro c hc
    ...
    have h_edge : step (NormalizedTM_v2 M) (Configuration.mk (Sum.inl M.acceptState) c.tapes c.positions) (Configuration.mk (Sum.inr 0) c.tapes c.positions) := by
      unfold step NormalizedTM_v2; aesop;
    -- By combining the paths, we can conclude that Sum.inr 0 � is� reachable from the initial configuration.
    have h_combined : Reachable (NormalizedTM_v2 M) (InitialConfig (NormalizedTM_v2 M) input) (Configuration.mk (Sum.inr 0) c.tapes c.positions) := by
      have h_step : step (NormalizedTM_v2 M) (Configuration.mk (Sum.inl c.state) c.tapes c.positions) (Configuration.mk (Sum.inr 0) c.tapes c.positions) := by
        aesop
...
-/
lemma NormalizedTM_v2_GoodTM {k : ℕ} {σ : Type} [DecidableEq σ] [Inhabited σ] [NeZero k]
  (M : TuringMachine k σ) (input : List σ) :
  GoodTM (NormalizedTM_v2 M) input := by
    refine' ⟨ _, _, _ ⟩;
    · unfold NormalizedTM_v2; aesop;
    · unfold Terminal;
      unfold step;
      unfold NormalizedTM_v2 at * ; aesop;
    · intro c hc hterm;
      contrapose! hterm;
      -- If the state is neither accept nor reject, then there must be some edge from this state.
      have h_edge : ∃ v w m, (c.state, v, fun i => c.tapes i (c.positions i).toNat, w, m) ∈ (NormalizedTM_v2 M).edges := by
        cases c : c.state <;> simp_all +decide [ NormalizedTM_v2 ];
...
-/
lemma NormalizedTM_v2_Terminal_iff {k : ℕ} {σ : Type} [DecidableEq σ] [Inhabited σ] [NeZero k]
  (M : TuringMachine k σ) (c' : Configuration k σ (NormalizedTM_v2 M).V) :
  Terminal (NormalizedTM_v2 M) c' ↔ c'.state = Sum.inr 0 ∨ c'.state = Sum.inr 1 := by
    constructor <;> intro h;
    · -- By definition of terminal, if c' is terminal, then there are no edges starting from c'.state.
      ...
      obtain ⟨u, hu⟩ : ∃ u, c'.state = Sum.inl u := by
        have h_sum_inl : ∀ x : (NormalizedTM_v2 M).V, x ≠ Sum.inr 0 ∧ x ≠ Sum.inr 1 → ∃ u, x = Sum.inl u := by
          intros x hx
      ...
      · by_cases hu_stuck : IsStuck M u (fun i => c'.tapes i (c'.positions i).toNat);
        · unfold Terminal at h;
          simp_all +decide [ step ];
          specialize h (Configuration.mk (Sum.inr 1) c'.tapes c'.positions) (fun i => c'.tapes i (c'.positions i).toNat) (fun i => c'.tapes ⟨i.val + 1, by omega⟩ (c'.positions ⟨i.val + 1, by omega⟩).toNat) (fun _ => Move.Stay) ; simp_all +decide [ NormalizedTM_v2 ];
        · unfold Terminal at h; simp_all +decide [ Terminal ] ;
          ...
          constructor;
          use w, m;
          unfold NormalizedTM_v2; simp +decide [ hu, hv ] ;
          unfold NextConfig; simp +decide [ hu ] ;
          tauto;
    · cases h <;> simp +decide [ *, Terminal ];
      · intro x hx; obtain ⟨ u, v, r, w, m, e_eq, he ⟩ := hx; simp_all +decide [ NormalizedTM_v2 ] ;
      · unfold step;
        unfold NormalizedTM_v2; aesop;
...
-/
lemma NormalizedTM_v2_Reachable_Lift {k : ℕ} {σ : Type} [DecidableEq σ] [Inhabited σ] [NeZero k]
  (M : TuringMachine k σ) (input : List σ) (c : Configuration k σ M.V) :
  Reachable M (InitialConfig M input) c →
  Reachable (NormalizedTM_v2 M) (InitialConfig (NormalizedTM_v2 M) input) (Configuration.mk (Sum.inl c.state) c.tapes c.positions) := by
    intro hc;
    -- By definition of NormalizedTM_v2, the edges of the normalized machine include the original edges of M, but with the states wrapped in Sum.inl.
    have h_edges : ∀ u v r w m, (u, v, r, w, m) ∈ M.edges → (Sum.inl u, Sum.inl v, r, w, m) ∈ (NormalizedTM_v2 M).edges := by
      intro u v r w m h
      simp [NormalizedTM_v2, h];
    induction hc;
    · constructor;
    · rename_i c' hc' ih;
      convert ih.tail _;
...
-/
lemma NormalizedTM_v2_AcceptsCoN_imp_AcceptsCoN {k : ℕ} {σ : Type} [DecidableEq σ] [Inhabited σ] [NeZero k]
  (M : TuringMachine k σ) (input : List σ) :
  AcceptsCoN (NormalizedTM_v2 M) input → AcceptsCoN M input := by
    contrapose!;
    unfold AcceptsCoN;
    simp +zetaDelta at *;
    intro c hc hterm hneq
    obtain ⟨c', hc', hterm'⟩ : ∃ c', Reachable (NormalizedTM_v2 M) (InitialConfig (NormalizedTM_v2 M) input) c' ∧ c'.state = Sum.inr 1 ∧ c'.positions = c.positions := by
      have h_reach : Reachable (NormalizedTM_v2 M) (InitialConfig (NormalizedTM_v2 M) input) (Configuration.mk (Sum.inl c.state) c.tapes c.positions) := by
        exact?;
      -- Since `c` is � terminal� in `M`, there are no edges in `M` from `c`. Thus, there are no "original edges" from `c'` in `NormalizedTM_v2 M`.
      have h_no_original_edges : ¬∃ v w m, (Sum.inl c.state, Sum.inl v, fun i => c.tapes i (c.positions i).toNat, w, m) ∈ (NormalizedTM_v2 M).edges := by
        unfold NormalizedTM_v2 at *; simp_all +decide [ Terminal ] ;
      ...
      -- Since `c` is terminal in `M`, there are no edges in `M` from `c`. Thus, there are no "original edges" from `c'` in `NormalizedTM_v2 M`. The only possible edges from `c'` are "accept edges" or "reject edges".
      have h_reject_edge : (Sum.inl c.state, Sum.inr 1, fun i => c.tapes i (c.positions i).toNat, fun i => c.tapes ⟨i.val + 1, by omega⟩ (c.positions ⟨i.val + 1, by omega⟩).toNat, fun _ => Move.Stay) ∈ (NormalizedTM_v2 M).edges := by
        unfold NormalizedTM_v2 at *; aesop;
      have h_reach_reject : Reachable (NormalizedTM_v2 M) (InitialConfig (NormalizedTM_v2 M) input) (NextConfig { state := Sum.inl c.state, tapes := c.tapes, positions := c.positions } (Sum.inr 1) (fun i => c.tapes ⟨i.val + 1, by omega⟩ (c.positions ⟨i.val + 1, by omega⟩).toNat) (fun _ => Move.Stay)) := by
    ...
    · intro x hx; unfold step at hx; simp_all +decide [ NormalizedTM_v2 ] ;
    · simp +decide [ NormalizedTM_v2 ]
...
-/
lemma NormalizedTM_v2_Reachable_Reject_Imp_Reachable_Stuck {k : ℕ} {σ : Type} [DecidableEq σ] [Inhabited σ] [NeZero k]
  (M : TuringMachine k σ) (input : List σ) (c' : Configuration k σ (NormalizedTM_v2 M).V) :
  Reachable (NormalizedTM_v2 M) (InitialConfig (NormalizedTM_v2 M) input) c' →
  c'.state = Sum.inr 1 →
  ∃ c, Reachable M (InitialConfig M input) c ∧ c.state ≠ M.acceptState ∧ IsStuck M c.state (fun i => c.tapes i (c.positions i).toNat) := by
    intro hc' hc'_state
    obtain ⟨c_prev, hc_prev, hc'_prev⟩ : ∃ c_prev, Reachable (NormalizedTM_v2 M) (InitialConfig (NormalizedTM_v2 M) input) c_prev ∧ step (NormalizedTM_v2 M) c_prev c' := by
      induction hc' <;> tauto;
    ...
    obtain ⟨u, hu⟩ : ∃ u : M.V, c_prev.state = Sum.inl u ∧ u ≠ M.acceptState ∧ IsStuck M u (fun i => c_prev.tapes i (c_prev.positions i).toNat) := by
      ...
      unfold NormalizedTM_v2 at hc'_prev; aesop;
    ...
    obtain ⟨c, hc⟩ : ∃ c : Configuration k σ M.V, Reachable M (InitialConfig M input) c ∧ c.state = u ∧ c.tapes = c_prev.tapes ∧ c.positions = c_prev.positions := by
      have := NormalizedTM_v2_Reachable_StrongCorrespondence M input c_prev hc_prev;
      grind;
    use c; aesop;
...
-/
lemma AcceptsCoN_imp_NormalizedTM_v2_AcceptsCoN {k : ℕ} {σ : Type} [DecidableEq σ] [Inhabited σ] [NeZero k]
  (M : TuringMachine k σ) (input : List σ) :
  AcceptsCoN M input → AcceptsCoN (NormalizedTM_v2 M) input := by
    intro h;
    unfold AcceptsCoN at *;
    intro c hc hcterminal; have := NormalizedTM_v2_Terminal_iff M c; simp_all +decide [ Terminal ] ;
    -- Since `c` is reachable and terminal in `NormalizedTM_v2 M`, by `NormalizedTM_v2_Reachable_Reject_Imp_Reachable_Stuck`, there exists `c'` reachable in `M` such that `c'.state ≠ M.acceptState` and `IsStuck M � c�'.state ...`.
    by_cases h_case : c.state = Sum.inr 1;
    · -- By `NormalizedTM_v2_Reachable_Reject_I �mp�_Reachable_Stuck`, there exists `c'` reachable in `M` such that `c'.state ≠ M.acceptState` and `IsStuck M c'.state ...`.
...
If the original Turing machine accepts an input in co-NSPACE, then the normalized Turing machine also accepts it in co-NSPACE.
-/
lemma AcceptsCoN_imp_NormalizedTM_v2_AcceptsCoN_v2 {k : ℕ} {σ : Type} [DecidableEq σ] [Inhabited σ] [NeZero k]
  (M : TuringMachine k σ) (input : List σ) :
  AcceptsCoN M input → AcceptsCoN (NormalizedTM_v2 M) input := by
    exact?
...
If the original Turing machine accepts an input in co-NSPACE, then the normalized Turing machine also accepts it in co-NSPACE.
-/
lemma AcceptsCoN_imp_NormalizedTM_v2_AcceptsCoN_v3 {k : ℕ} {σ : Type} [DecidableEq σ] [Inhabited σ] [NeZero k]
  (M : TuringMachine k σ) (input : List σ) :
  AcceptsCoN M input → AcceptsCoN (NormalizedTM_v2 M) input := by
    -- Apply the lemma that states if the original machine accepts in co-NSPACE, then the normalized machine also accepts in co-NSPACE.
    apply AcceptsCoN_imp_NormalizedTM_v2_AcceptsCoN_v2

/-
If the normalized Turing machine reaches the reject state, that configuration is terminal.
-/
lemma NormalizedTM_v2_Reject_Terminal {k : ℕ} {σ : Type} [DecidableEq σ] [Inhabited σ] [NeZero k]
  (M : TuringMachine k σ) (input : List σ) (c : Configuration k σ (NormalizedTM_v2 M).V) :
  Reachable (NormalizedTM_v2 M) (InitialConfig (NormalizedTM_v2 M) input) c →
  c.state = (NormalizedTM_v2 M).rejectState →
  Terminal (NormalizedTM_v2 M) c := by
    intros hc hc_reject
    have h_terminal : c.state = (NormalizedTM_v2 M).rejectState → Terminal (NormalizedTM_v2 M) c := by
      intros hc_reject
      apply NormalizedTM_v2_Terminal_iff M c |>.2;
...
-/
lemma NormalizedTM_v2_AcceptsCoN_iff_Not_AcceptsN_SwapStates {k : ℕ} {σ : Type} [DecidableEq σ] [Inhabited σ] [NeZero k]
  (M : TuringMachine k σ) (input : List σ) :
  AcceptsCoN (NormalizedTM_v2 M) input ↔ ¬ AcceptsN (SwapStates (NormalizedTM_v2 M)) input := by
    apply Iff.intro;
    · intro h;
      apply SwapStates_AcceptsCoN_imp_Not_AcceptsN;
      · unfold SwapStates NormalizedTM_v2; aesop;
      · have h_terminal : ∀ c', Reachable (NormalizedTM_v2 M) (InitialConfig (NormalizedTM_v2 M) input) c' → c'.state = (NormalizedTM_v2 M).rejectState → Terminal (NormalizedTM_v2 M) c' := by
          exact?;
        unfold SwapStates at *; aesop;
      · convert h using 1;
    · intro h_not_accept
      have h_terminal : ∀ c, Reachable (NormalizedTM_v2 M) (InitialConfig (NormalizedTM_v2 M) input) c → Terminal (NormalizedTM_v2 M) c → c.state = (NormalizedTM_v2 M).acceptState := by
        intro c hc hc_terminal
        have h_state : c.state = (NormalizedTM_v2 M).acceptState ∨ c.state = (NormalizedTM_v2 M).rejectState := by
          apply (NormalizedTM_v2_GoodTM M input).2.2 c hc hc_terminal;
...
-/
lemma NormalizedTM_v2_AcceptsCoN_iff {k : ℕ} {σ : Type} [DecidableEq σ] [Inhabited σ] [NeZero k]
  (M : TuringMachine k σ) (input : List σ) :
  AcceptsCoN (NormalizedTM_v2 M) input ↔ AcceptsCoN M input := by
    apply Iff.intro;
    · exact?;
...
theorem InNSPACE_O_eq_InCoNSPACE_O
  ...
  InNSPACE_O f L ↔ InCoNSPACE_O f L := by
    ...
    · -- By definition of $InNSPACE_O �$,� there exists some constant $c > 0$ such that $InNSPACE (fun n => c * f n) L$.
      ...
      use c';
      obtain ⟨ k, hk, M, hM ⟩ := hc';
      refine' ⟨ hc'_pos, k, hk, SwapStates ( NormalizedTM_v2 M ), _, _ ⟩ <;> simp_all +decide [ LanguageRecognizedByN, LanguageRecognizedByCoN ];
      · exact SwapStates_SpaceBounded _ _ ( NormalizedTM_v2_SpaceBounded _ _ hM.1 );
      · ext input; specialize hM; have := hM.2; simp_all +decide [ Set.ext_iff, LanguageComplement ] ;
        have h_swap : AcceptsCoN (SwapStates (NormalizedTM_v2 M)) input ↔ ¬ AcceptsN (NormalizedTM_v2 M) input := by
          apply SwapStates_AcceptsCoN_iff_Not_AcceptsN;
          · exact ne_of_apply_ne ( fun x => x ) ( by simp +decide [ NormalizedTM_v2 ] );
          · exact fun c hc hc' => by rw [ NormalizedTM_v2_Terminal_iff ] ; aesop;
          · exact fun c hc₁ hc₂ => by have := NormalizedTM_v2_Terminal_iff M c; aesop;
        have h_normalized : AcceptsN (NormalizedTM_v2 M) input ↔ AcceptsN M input := by
          exact?;
        aesop;
    · -- By definition of `InCoNSPACE_O �`, � there exists a machine `M` and a constant `c` such that `M` accepts `L` in co-NSPACE with space bound `c * f`.
      ...
      -- By definition of `LanguageRecognizedByCoN`, there exists a Turing machine `M'` and a constant `c'` such that `M'` accepts `L` in co-NSPACE with space bound `c' * f`. Since `LanguageRecognizedByCoN M' = L`, we have `LanguageRecognizedByN (SwapStates (NormalizedTM_v2 M')) = L`.
      have hL_swap : LanguageRecognizedByN (SwapStates (NormalizedTM_v2 M')) = LanguageComplement L := by
        have hL_swap : ∀ input, AcceptsCoN (NormalizedTM_v2 M') input ↔ ¬ AcceptsN (SwapStates (NormalizedTM_v2 M')) input := by
          apply NormalizedTM_v2_AcceptsCoN_iff_Not_AcceptsN_SwapStates;
        have hL_swap : ∀ input, AcceptsCoN (NormalizedTM_v2 M') input ↔ AcceptsCoN M' input := by
          exact?;
        unfold LanguageRecognizedByN LanguageRecognizedByCoN LanguageComplement at *; aesop;
      -- Since `LanguageRecognizedByN (SwapStates (NormalizedTM_v2 M')) = LanguageComplement L`, we have `InNSPACE_O f (LanguageComplement L)`.
      have hL_complement : InNSPACE_O f (LanguageComplement L) := by
        use c;
        refine' ⟨ hc_pos, _, _ ⟩;
        exact M;
        refine' ⟨ hk, SwapStates ( NormalizedTM_v2 M' ), _, _ ⟩;
        · exact SwapStates_SpaceBounded _ _ ( NormalizedTM_v2_SpaceBounded _ _ hM' );
        · exact hL_swap;
